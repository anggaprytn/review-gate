...
pub fn qa_checklist_marker(project_path: &str, mr_iid: u64) -> String {
    format!(
        "<!-- reviewgate:qa-checklist project=\"{}\" mr=\"{}\" -->",
        escape_marker_attr(project_path),
        mr_iid
    )
}
...
pub fn build_qa_checklist_note_body(
    markdown: &str,
    project_path: &str,
    mr_iid: u64,
    max_chars: usize,
    ...
    let body = format!(
        "{markdown}\n\n{REVIEWGATE_ATTRIBUTION}\n\n{marker}\n",
        markdown = strip_terminal_ai_footer(markdown).trim_end(),
        marker = qa_checklist_marker(project_path, mr_iid),
    );
...
fn strip_terminal_ai_footer(markdown: &str) -> &str {
    ...
    let trimmed = trimmed
        ...
        .unwrap_or(trimmed);
    trimmed
        .strip_suffix("[AI generated by ReviewGate]")
        .map(str::trim_end)
        .unwrap_or(trimmed)
}
...
mod tests {
    use super::{
        ...
        publish_summary_with, qa_checklist_marker, reviewgate_note_count, select_publish_target,
        summary_marker, truncate_note_body, verification_marker,
    };
    use crate::{
    ...
    #[test]
    fn generates_stable_reviewgate_qa_checklist_marker() {
        assert_eq!(
            qa_checklist_marker("group/repo", 59),
            r#"<!-- reviewgate:qa-checklist project="group/repo" mr="59" -->"#
        );
    }
    ...
    #[test]
    fn detects_existing_qa_checklist_note_with_generic_selector() {
        let marker = qa_checklist_marker("group/repo", 59);
        let notes = vec![
            note(1, &marker, true, "2026-01-01T00:00:00Z"),
            note(2, &marker, false, "2026-01-02T00:00:00Z"),
            note(3, "ordinary note", false, "2026-01-03T00:00:00Z"),
        ];
    ...
    fn builds_published_body_with_marker_and_metadata() {
        ...
        assert!(body.contains("Open priority findings: 7"));
    ...
    #[test]
    fn builds_qa_checklist_body_with_marker_and_attribution() {
        let body = build_qa_checklist_note_body(
            "# ReviewGate Human QA Checklist\n\n## QA Summary\n\nManual checks recommended: 1\n\n[AI generated by ReviewGate]\n",
            "group/repo",
            59,
            10_000,
        )
        ...
        assert!(body.contains("# ReviewGate Human QA Checklist"));
        assert!(body.contains("Manual checks recommended: 1"));
        assert!(
            body.ends_with("<!-- reviewgate:qa-checklist project=\"group/repo\" mr=\"59\" -->\n")
        );
        assert_eq!(body.matches("[AI generated by ReviewGate]").count(), 1);
    }
